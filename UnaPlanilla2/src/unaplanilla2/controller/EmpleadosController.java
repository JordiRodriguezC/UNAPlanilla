...
import unaplanilla2.util.BindingUtils;
...
public class EmpleadosController extends Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if(AppContext.getInstance().get("Empleado")==null){
            ...
            empleado = (EmpleadoDto) AppContext.getInstance().get("Empleado");
            bindEmpleado(false);
        }
         indicarRequeridos();
    ...
    private void buscarEmpleado(ActionEvent event) {
        FlowController.getInstance().goViewInWindow("Buscar");
    ...
    private void eliminarEmpleado(ActionEvent event) {
        try {
            if (empleado.getEmpId() == null) {
                ...
                EmpleadoService service = new EmpleadoService();
                Respuesta resp = service.eliminarEmpleado(empleado.getEmpId());
                  if (!resp.getEstado()) {
                    new Mensaje().showModal(Alert.AlertType.ERROR, "Eliminar empleado", getStage(), resp.getMensaje());
                } else {
